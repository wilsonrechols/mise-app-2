...
const STORAGE_KEY = 'mise:state:v2';
...
// Slot types for eating out
const SLOT_TYPES = ['recipe','leftover','eating_out'];
...
const DEFAULT_STATE = {
  ...
  cookLog:{},       // { recipeId: [{date, rating, notes}] }
  mealHistory:[],   // [{date, slotType, recipeId?, label, note}]
};
...
async function callAI(prompt,options={}){
  const model = options.smart ? 'claude-sonnet-4-20250514' : 'claude-haiku-4-5-20251001';
...
function RecipeCard({recipe,onClick}){
  ...
  return(
    <button onClick={onClick} className="rcs text-left bg-white border border-stone-200 rounded-2xl overflow-hidden transition-all hover:-translate-y-0.5 w-full">
      {/* AI photo if present */}
      {recipe.photoUrl&&(
        <div className="w-full h-36 overflow-hidden bg-stone-100">
...
function RecipeForm({initial,onSave,onCancel}){
  ...
  async function generatePhoto(){
    ...
    try{
      // Ask AI to give us a Unsplash-style search description for the dish
      const desc=await callAI(`For the dish "${recipe.name}" (${recipe.cuisine||''}), give me a single short phrase (3-6 words, no quotes) suitable as an Unsplash photo search for a beautiful food photography image of this dish. Reply with ONLY the phrase.`);
      const query=encodeURIComponent(desc.trim().slice(0,80));
      // Use Unsplash source for a deterministic food photo
      const photoUrl=`https://source.unsplash.com/800x500/?${query},food,dish`;
      u('photoUrl',photoUrl);
        ...
        {recipe.photoUrl?(
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={generatePhoto} disabled={generatingPhoto||!recipe.name.trim()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-stone-200 text-sm text-stone-600 hover:border-stone-400 disabled:opacity-50">
                {generatingPhoto?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<RefreshCw className="w-3.5 h-3.5"/>} Regenerate
              </button>
              <button type="button" onClick={()=>u('photoUrl',null)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-stone-200 text-sm text-stone-600 hover:border-red-200 hover:text-red-600">
          ...
          <button type="button" onClick={generatePhoto} disabled={generatingPhoto||!recipe.name.trim()} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-stone-300 text-sm text-stone-500 hover:border-orange-400 hover:text-orange-700 disabled:opacity-50 w-full justify-center">
            {generatingPhoto?<><Loader2 className="w-4 h-4 animate-spin"/> Generating photo…</>:<><ImageIcon className="w-4 h-4"/> Auto-generate photo with AI</>}
          </button>
        )}
      </div>
...
// ============================================================
// FEATURE 1: Eating Out slot type
// FEATURE 4: Swipe between days on mobile
// ============================================================
...
function WeekPlanView({recipes,mealPlan,currentWeek,setCurrentWeek,cookedSlots,onPickSlot,onClearSlot,onSelectRecipe,onMarkCooked,onSetMultiplier}){
  ...
  function onTouchEnd(e){
    ...
    if(Math.abs(dx)>50){
      if(dx<0)setSelDayIdx(i=>Math.min(i+1,DAYS.length-1));
      else setSelDayIdx(i=>Math.max(i-1,0));
    }
  ...
  function renderDay(d){
    ...
    return(
      <div className="bg-white border border-stone-200 rounded-2xl p-3">
        ...
        <div className="space-y-2">
          {MEALS.map(m=>{
            const slot=normalizeSlot(dp[m]);
            const isEatingOut=slot?.slotType==='eating_out';
            const isLO=!!slot?.leftoverFrom;
            const origin=isLO?getOriginSlot(week,slot):slot;
                ...
                {isEatingOut?(
                  <div className="flex items-center justify-between gap-1">
                    <div className="flex items-center gap-1.5">
                      <UtensilsCrossed className="w-3.5 h-3.5 text-amber-600" strokeWidth={1.75}/>
                      <span className="font-display text-sm text-amber-800">{slot.label||'Eating out'}</span>
                    </div>
  ...
  return(
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div><h2 className="font-display text-4xl tracking-tight">This week</h2><p className="text-stone-600 text-sm mt-1">{formatWeekRange(currentWeek)}</p></div>
        <div className="flex items-center gap-1">
      ...
      <div className="lg:hidden">
        ...
        {/* Swipe area */}
        <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} className="relative overflow-hidden">
          {renderDay(sel)}
          {/* Swipe hint arrows */}
          <div className="flex justify-between mt-3 px-1">
            <button onClick={()=>setSelDayIdx(i=>Math.max(i-1,0))} disabled={selDayIdx===0} className="p-1.5 rounded-full text-stone-400 disabled:opacity-0 hover:bg-stone-100"><ChevronLeft className="w-4 h-4"/></button>
            <span className="text-xs text-stone-400 self-center">swipe to navigate</span>
            <button onClick={()=>setSelDayIdx(i=>Math.min(i+1,DAYS.length-1))} disabled={selDayIdx===DAYS.length-1} className="p-1.5 rounded-full text-stone-400 disabled:opacity-0 hover:bg-stone-100"><ChevronRight className="w-4 h-4"/></button>
          </div>
        </div>
      </div>
      {/* Desktop: all days */}
...
// ---------- Recipe Picker (updated with eating out option) ----------
function RecipePickerModal({recipes,recipeMap,title,currentWeekPlan,targetSlot,onPick,onPickLeftover,onPickEatingOut,onClose}){
  ...
  return(
    <Modal onClose={onClose}>
      ...
      <div className="flex gap-1 mb-4 p-1 bg-stone-100 rounded-full overflow-x-auto">
        {modes.map(md=><button key={md.id} onClick={()=>setMode(md.id)} className={`flex-1 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${mode===md.id?'bg-white text-stone-900 shadow-sm':'text-stone-600'}`}>{md.label}</button>)}
      </div>
      {mode==='library'&&<>
        <input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search recipes..." className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-500 mb-4"/>
        <div className="max-h-96 overflow-y-auto space-y-1">
          {filtered.length===0&&<p className="text-sm text-stone-500 py-6 text-center">No recipes match.</p>}
          {filtered.map(r=><button key={r.id} onClick={()=>onPick(r.id)} className="w-full text-left p-3 rounded-lg hover:bg-stone-100 flex items-center justify-between"><div><div className="font-display text-base">{r.name}</div><div className="text-xs text-stone-500">{r.cuisine} · {((r.prepTime||0)+(r.cookTime||0))}m</div></div>{r.rating==='up'&&<ThumbsUp className="w-3.5 h-3.5 text-emerald-600" fill="currentColor"/>}</button>)}
        </div>
      </>}
      {mode==='leftover'&&<div className="max-h-96 overflow-y-auto space-y-1">
        {origins.length===0&&<p className="text-sm text-stone-500 py-6 text-center">No cooked meals this week to use as leftovers.</p>}
        {origins.map(o=>{const r=recipeMap[o.recipeId];if(!r)return null;return(<button key={`${o.day}_${o.meal}`} onClick={()=>onPickLeftover(o.day,o.meal)} className="w-full text-left p-3 rounded-lg hover:bg-stone-100"><div className="text-[10px] uppercase tracking-wider text-stone-500">{o.dayLabel} · {o.meal}</div><div className="font-display text-base">{r.name}</div></button>);})}
      </div>}
      {mode==='eating_out'&&<div className="space-y-4">
        <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          <UtensilsCrossed className="w-6 h-6 text-amber-600 flex-shrink-0" strokeWidth={1.5}/>
          <div><p className="text-sm font-medium text-amber-900">Eating out</p><p className="text-xs text-amber-700">Mark this slot as a restaurant meal</p></div>
        </div>
        <div>
          <label className="text-xs uppercase tracking-wider text-stone-500 font-medium mb-1.5 block">Label (optional)</label>
          <input value={eoLabel} onChange={e=>setEoLabel(e.target.value)} placeholder="e.g. Date night, Tacos, Birthday dinner…" className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-500" onKeyDown={e=>e.key==='Enter'&&onPickEatingOut(eoLabel||'Eating out')}/>
        </div>
        <button onClick={()=>onPickEatingOut(eoLabel||'Eating out')} className="w-full py-2.5 rounded-full bg-amber-600 text-white text-sm hover:bg-amber-700 flex items-center justify-center gap-2"><UtensilsCrossed className="w-4 h-4"/> Set as eating out</button>
      </div>}
    </Modal>
  );
}
...
// ============================================================
// FEATURE 5: Grocery list sharing — native share sheet
// ============================================================
function GroceryView({recipes,mealPlan,currentWeek,setCurrentWeek,pantry,checks,manualItems,onToggleCheck,onAddManualItem,onRemoveManualItem}){
  ...
  function buildShareText(){
    ...
    for(const cat of CATEGORIES.filter(c=>grocery[c]?.length>0)){
      lines.push(`${cat.toUpperCase()}`);
      for(const item of grocery[cat]){const checked=!!checks[item.key];lines.push(`${checked?'✓':'-'} ${formatQuantity(item.quantity)}${item.unit?' '+item.unit:''} ${item.name}`);}
      lines.push('');
    }
    return lines.join('\n').trim();
  }
  ...
  async function shareList(){
    ...
    if(navigator.share){
      try{await navigator.share({title:'Mise grocery list',text});setShareStatus('shared');}
  ...
  function fallbackCopy(text){
    navigator.clipboard.writeText(text).then(()=>setShareStatus('copied')).catch(()=>{const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);setShareStatus('copied');});
  }
  ...
  return(
    <div>
      ...
      {/* Share button */}
      {ti>0&&<div className="mb-4">
        <button onClick={shareList} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm transition-colors ${shareStatus==='shared'||shareStatus==='copied'?'bg-emerald-50 text-emerald-700 border border-emerald-200':'bg-white border border-stone-200 text-stone-600 hover:border-stone-400'}`}>
          {shareStatus==='shared'?<><Check className="w-4 h-4" strokeWidth={3}/> Shared!</>:shareStatus==='copied'?<><Check className="w-4 h-4" strokeWidth={3}/> Copied!</>:<><Share2 className="w-4 h-4"/> Share list</>}
        </button>
      </div>}
...
// ============================================================
// FEATURE 2: Cook Log / Review badge
// ============================================================
  ...
  return(
    <div className="bg-white border border-stone-200 rounded-2xl p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wider text-orange-700 mb-1">Review cook</p>
          <h3 className="font-display text-xl font-medium">{recipe.name}</h3>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-full hover:bg-stone-100 text-stone-400"><X className="w-4 h-4"/></button>
      </div>
      <div>
        <p className="text-xs uppercase tracking-wider text-stone-500 font-medium mb-2">How did it go?</p>
        <div className="flex gap-2">
          {stars.map(s=>(
            <button key={s} onClick={()=>setRating(s)} className={`w-9 h-9 rounded-full border flex items-center justify-center text-lg transition-colors ${rating>=s?'bg-orange-50 border-orange-300 text-orange-600':'border-stone-200 text-stone-300 hover:border-stone-400'}`}>★</button>
          ))}
        </div>
      </div>
      <div>
        <p className="text-xs uppercase tracking-wider text-stone-500 font-medium mb-2">Notes (optional)</p>
        <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} placeholder="Added extra garlic, used thighs instead of breasts…" className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-500 resize-none"/>
      </div>
      <div className="flex gap-2 justify-end">
        <button onClick={onClose} className="px-4 py-2 rounded-full text-sm text-stone-600 hover:bg-stone-100">Skip</button>
        <button onClick={submit} className="px-4 py-2 rounded-full bg-stone-900 text-stone-50 text-sm flex items-center gap-1.5"><Check className="w-3.5 h-3.5"/> Save review</button>
      </div>
    </div>
  );
}
...
function QuickLogModal({onClose,onLog,recipes}){
  ...
  async function handleParse(){
    ...
    setLoading(true);
    try{
      const result=await callAI(`Given this free-text meal log: "${text}"
...
Return ONLY JSON: {"matchedRecipe": "string or null", "label": "string"}`);
      setParsed(extractJSON(result));
    }catch(e){setParsed({matchedRecipe:null,label:text.trim()});}
    setLoading(false);
  }

  function handleLog(){
    if(!parsed&&!text.trim())return;
    const label=parsed?.label||text.trim();
    const matchedRecipe=parsed?.matchedRecipe?recipes.find(r=>r.name.toLowerCase()===parsed.matchedRecipe.toLowerCase()):null;
    onLog({day,meal,label,recipeId:matchedRecipe?.id||null,text:text.trim(),date:Date.now()});
    onClose();
  }
  ...
  return(
    <Modal onClose={onClose}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-display text-2xl">Quick log</h3>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex gap-2">
          <input
            ...
            placeholder="e.g. made a sandwich, chicken tacos, spaghetti…"
            className="flex-1 px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-500"
          />
          {!parsed&&<button onClick={handleParse} disabled={!text.trim()||loading} className="px-4 py-2 rounded-lg bg-stone-900 text-stone-50 text-sm disabled:opacity-50 flex items-center gap-1.5">{loading?<Loader2 className="w-4 h-4 animate-spin"/>:<Sparkles className="w-4 h-4"/>}</button>}
        </div>
        {parsed&&(
          <div className="p-3 bg-stone-50 border border-stone-200 rounded-lg">
            {parsed.matchedRecipe?(
              <p className="text-sm">Matched to: <span className="font-medium text-emerald-700">{parsed.matchedRecipe}</span></p>
            ):(
              <p className="text-sm">Log as: <span className="font-medium">{parsed.label}</span></p>
            )}
          </div>
        )}
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs uppercase tracking-wider text-stone-500 font-medium mb-1.5 block">When</label>
            <select value={day} onChange={e=>setDay(e.target.value)} className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none">
          ...
          <div>
            ...
            <select value={meal} onChange={e=>setMeal(e.target.value)} className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none">
              {MEALS.map(m=><option key={m} value={m}>{m.charAt(0).toUpperCase()+m.slice(1)}</option>)}
            </select>
          </div>
        </div>
        <div className="flex gap-2 justify-end pt-1">
          <button onClick={onClose} className="px-4 py-2 rounded-full text-sm text-stone-600 hover:bg-stone-100">Cancel</button>
          <button onClick={handleLog} disabled={!text.trim()&&!parsed} className="px-5 py-2 rounded-full bg-orange-700 text-white text-sm hover:bg-orange-800 flex items-center gap-2 disabled:opacity-50"><Zap className="w-4 h-4"/> Log meal</button>
        </div>
      </div>
    </Modal>
  );
...
// ============================================================
// FEATURE 8: Meal History (30 days) — shown in Insights tab
// FEATURE 2: Cook Log displayed in Insights
// ============================================================
function MealHistorySection({mealHistory,recipes}){
  ...
  if(recent.length===0)return(
    <div className="bg-white border border-stone-200 rounded-2xl p-5">
      <h3 className="font-display text-lg mb-2 flex items-center gap-2"><History className="w-4 h-4 text-stone-500"/> Meal history</h3>
      <p className="text-sm text-stone-500">No meals logged yet. Use quick log or mark meals as cooked.</p>
    </div>
  );
  // Group by date
          ...
          <div key={date}>
            ...
            <div className="space-y-1">
              {entries.map((h,i)=>{
                ...
                return(
                  <div key={i} className="flex items-center gap-3 py-1.5 px-2 rounded-lg bg-stone-50">
...
function InsightsView({recipes,onSaveRecipe,mealHistory,cookLog}){
  const[suggestions,setSuggestions]=useState([]);const[loading,setLoading]=useState(false);const[error,setError]=useState(null);const[showHistory,setShowHistory]=useState(false);
  ...
  return(
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <StatCard label="Recipes" value={stats.total}/>
        <StatCard label="Cooked this month" value={stats.cookedThisMonth}/>
        <StatCard label="Avg total time" value={`${stats.avgTime}m`}/>
        <StatCard label="Top rated" value={stats.topRated.length}/>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white border border-stone-200 rounded-2xl p-5"><h3 className="font-display text-lg mb-3">Cuisine breakdown</h3><div className="space-y-2">{stats.cuisineRanked.slice(0,6).map(([c,n])=>(<div key={c} className="flex items-center gap-3"><span className="text-sm text-stone-600 w-32 truncate">{c}</span><div className="flex-1 bg-stone-100 rounded-full h-1.5 overflow-hidden"><div className="h-full bg-orange-700" style={{width:`${(n/mx)*100}%`}}/></div><span className="text-xs text-stone-500 w-6 text-right">{n}</span></div>))}</div></div>
        <div className="bg-white border border-stone-200 rounded-2xl p-5"><h3 className="font-display text-lg mb-3">Most-used ingredients</h3><div className="flex flex-wrap gap-2">{stats.ingredientRanked.map(([i,n])=><span key={i} className="text-xs px-2.5 py-1 bg-stone-100 text-stone-700 rounded-full">{i} <span className="text-stone-400">·{n}</span></span>)}</div></div>
      </div>
      {(stats.mostCooked.length>0||stats.topRated.length>0)&&<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {stats.mostCooked.length>0&&<div className="bg-white border border-stone-200 rounded-2xl p-5"><h3 className="font-display text-lg mb-3 flex items-center gap-2"><Flame className="w-4 h-4 text-orange-700"/> Most cooked</h3><div className="space-y-1.5">{stats.mostCooked.map(r=><div key={r.id} className="flex items-center justify-between text-sm"><span className="font-display">{r.name}</span><span className="text-stone-500">×{r.cookCount}</span></div>)}</div></div>}
        {stats.topRated.length>0&&<div className="bg-white border border-stone-200 rounded-2xl p-5"><h3 className="font-display text-lg mb-3 flex items-center gap-2"><ThumbsUp className="w-4 h-4 text-emerald-600"/> Loved</h3><div className="space-y-1.5">{stats.topRated.map(r=><div key={r.id} className="text-sm font-display">{r.name}</div>)}</div></div>}
      </div>}
      {/* Meal history section */}
      <div className="mb-6">
        <MealHistorySection mealHistory={mealHistory} recipes={recipeMap}/>
      </div>
      <div className="bg-white border border-stone-200 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2"><h3 className="font-display text-xl flex items-center gap-2"><Sparkles className="w-4 h-4 text-orange-700"/> Recommendations</h3><button onClick={suggest} disabled={loading} className="px-4 py-1.5 rounded-full bg-stone-900 text-stone-50 text-sm disabled:opacity-50 flex items-center gap-2">{loading?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<RefreshCw className="w-3.5 h-3.5"/>}{loading?'Thinking…':suggestions.length?'Refresh':'Suggest recipes'}</button></div>
        {error&&<p className="text-sm text-red-600 mb-3">{error}</p>}
        {suggestions.length===0&&!loading&&<p className="text-sm text-stone-500">Based on your library, we'll suggest 5 new recipes you'd likely enjoy.</p>}
        <div className="space-y-3 mt-3">{suggestions.map((s,i)=><div key={i} className="border border-stone-200 rounded-xl p-4"><div className="flex items-start justify-between gap-3 mb-2"><div className="flex-1"><div className="text-xs uppercase tracking-wider text-orange-700 mb-1">{s.cuisine} · {s.totalTime}m</div><h4 className="font-display text-lg leading-tight">{s.name}</h4></div><button onClick={()=>expand(s)} className="px-3 py-1.5 rounded-full bg-emerald-700 text-white text-xs hover:bg-emerald-800 flex items-center gap-1 flex-shrink-0"><Plus className="w-3 h-3"/> Save</button></div><p className="text-sm text-stone-600 mb-2">{s.why}</p><div className="flex flex-wrap gap-1.5">{(s.highlights||[]).map(h=><span key={h} className="text-xs px-2 py-0.5 bg-stone-100 text-stone-600 rounded-full">{h}</span>)}</div></div>)}</div>
      </div>
    </div>
  );
}
...
// ============================================================
// FEATURE 3: Duplicate recipe button in detail modal
// ============================================================
function RecipeDetailModal({recipe,onClose,onEdit,onDelete,onCook,onRate,onDuplicate,onSaveCookLog}){
    ...
    <Modal onClose={onClose} wide>
      ...
      <div className="flex items-start justify-between mb-4 gap-3">
        ...
        <div className="flex gap-1">
          {/* Duplicate button */}
          <button onClick={onDuplicate} title="Duplicate recipe" className="p-2 rounded-full hover:bg-stone-100 text-stone-600"><Copy className="w-4 h-4"/></button>
          <button onClick={onEdit} className="p-2 rounded-full hover:bg-stone-100 text-stone-600"><Edit2 className="w-4 h-4"/></button>
          <button onClick={onDelete} className="p-2 rounded-full hover:bg-red-50 text-stone-600 hover:text-red-600"><Trash2 className="w-4 h-4"/></button>
        </div>
      </div>
...
export default function App(){
  ...
  // Feature 2: pending review after cook
  const[pendingReview,setPendingReview]=useState(null); // recipeId

  const recipes=state.recipes;
  const recipeList=useMemo(()=>Object.values(recipes),[recipes]);

  // ---- unreviewed cook count for badge ----
  const unreviewedCount=useMemo(()=>{
    const cookLog=state.cookLog||{};
    let count=0;
    for(const r of recipeList){
      if(!r.lastCooked)continue;
      const age=(Date.now()-r.lastCooked)/86400000;
      if(age>7)continue; // only flag within 7 days
      const logs=(cookLog[r.id]||[]);
      const lastLogDate=logs.length>0?logs[logs.length-1].date:0;
      if(r.lastCooked>lastLogDate)count++;
    }
    return count;
  ...
  function logCook(id){
    setState(s=>({...s,recipes:{...s.recipes,[id]:{...s.recipes[id],cookCount:(s.recipes[id].cookCount||0)+1,lastCooked:Date.now()}}}));
    // add to meal history
    const recipe=recipes[id];
    if(recipe){
      addMealHistory({date:Date.now(),recipeId:id,label:recipe.name,meal:'dinner',slotType:'recipe'});
    }
  }
  ...
  function duplicateRecipe(id){
    const orig=recipes[id];if(!orig)return;
    const newId=generateId();
    const copy={...orig,id:newId,name:`${orig.name} (copy)`,createdAt:Date.now(),cookCount:0,lastCooked:null,rating:null};
    setState(s=>({...s,recipes:{...s.recipes,[newId]:copy}}));
    return newId;
  }

  // Feature 2: save cook log entry
  function saveCookLog(recipeId,logEntry){
    setState(s=>{const prev=s.cookLog||{};const entries=prev[recipeId]||[];return{...s,cookLog:{...prev,[recipeId]:[...entries,logEntry]}};});
  }

  // Feature 6/8: meal history
  function addMealHistory(entry){
    setState(s=>{const hist=(s.mealHistory||[]);return{...s,mealHistory:[...hist,{...entry,id:generateId()}]};});
  }

  // Feature 6: quick log handler
  function handleQuickLog({day,meal,label,recipeId,text,date}){
    // Resolve day key
    let dateISO;
    if(day==='today')dateISO=todayISO();
    else if(day==='yesterday')dateISO=daysAgo(1);
    else dateISO=null;

    // Add to meal history
    addMealHistory({date,meal,label:label||text,recipeId:recipeId||null,slotType:'recipe'});

    // If it matches a recipe, log the cook
    if(recipeId){
      setState(s=>({...s,recipes:{...s.recipes,[recipeId]:{...s.recipes[recipeId],cookCount:(s.recipes[recipeId]?.cookCount||0)+1,lastCooked:date}}}));
    }

    // Optionally fill week plan slot for today/yesterday
    if(dateISO){
      const ws=getWeekStart(new Date(dateISO));
      const dayKey=DAYS.find(d=>{
        const t=new Date(dateISO);
        const dkm=['sun','mon','tue','wed','thu','fri','sat'];
        return d.key===dkm[t.getDay()];
      })?.key;
      if(dayKey&&recipeId){
        planMeal(ws,dayKey,meal,recipeId);
      }
    }
  }
  ...
  function markSlotCooked(week,day,meal,recipeId,currentlyCooked){const slotKey=`${day}_${meal}`;setState(s=>{const wc=s.cookedSlots[week]||{},newCooked={...wc};if(currentlyCooked){delete newCooked[slotKey];return{...s,cookedSlots:{...s.cookedSlots,[week]:newCooked}};}newCooked[slotKey]={date:Date.now()};const r=s.recipes[recipeId];const ur=r?{...s.recipes,[recipeId]:{...r,cookCount:(r.cookCount||0)+1,lastCooked:Date.now()}}:s.recipes;return{...s,cookedSlots:{...s.cookedSlots,[week]:newCooked},recipes:ur};});
    ...
    if(recipe&&!currentlyCooked){addMealHistory({date:Date.now(),recipeId,label:recipe.name,meal,slotType:'recipe'});}
  }
  ...
  return(
    <div className="paper-bg min-h-screen text-stone-900">
      <header className="border-b border-stone-200 bg-stone-50/80 backdrop-blur sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={()=>setShowQuickLog(true)} title="Quick log a meal" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-700 text-white text-xs hover:bg-orange-800">
              <Zap className="w-3.5 h-3.5"/> <span className="hidden sm:inline">Quick log</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-6">
          {mobTabs.map(tab=>{const Icon=tab.icon;const active=view===tab.id;return(
            <button key={tab.id} onClick={()=>setView(tab.id)} className={`relative flex flex-col items-center justify-center gap-0.5 py-2 px-1 ${active?'text-orange-700':'text-stone-500 hover:text-stone-700'}`}>
              ...
              {tab.badge&&<span className="absolute top-1 right-3 w-3.5 h-3.5 rounded-full bg-orange-600 text-white text-[8px] flex items-center justify-center font-bold animate-pulse">{unreviewedCount}</span>}
            </button>
          );})}
        </div>
      </nav>
      ...
      {selectedRecipe&&!editingRecipe&&<RecipeDetailModal
        ...
        onDelete={()=>{if(confirm(`Delete "${selectedRecipe.name}"?`)){deleteRecipe(selectedRecipe.id);setSelectedRecipeId(null);}}}
        onCook={()=>logCook(selectedRecipe.id)}
        onRate={rating=>setRating(selectedRecipe.id,rating)}
        onDuplicate={()=>{const newId=duplicateRecipe(selectedRecipe.id);setSelectedRecipeId(null);}}
      ...
      {planTarget&&<RecipePickerModal
        recipes={recipeList}
        recipeMap={recipes}
        title={`${planTarget.day} ${planTarget.meal}`}
        currentWeekPlan={state.mealPlan[planTarget.week]||{}}
        targetSlot={{day:planTarget.day,meal:planTarget.meal}}
        onPick={id=>{planMeal(planTarget.week,planTarget.day,planTarget.meal,id);setPlanTarget(null);}}
        onPickLeftover={(od,om)=>{planMeal(planTarget.week,planTarget.day,planTarget.meal,{leftoverFrom:{day:od,meal:om}});setPlanTarget(null);}}
        onPickEatingOut={(label)=>{planMeal(planTarget.week,planTarget.day,planTarget.meal,{slotType:'eating_out',label});setPlanTarget(null);}}
