...
function listOriginSlots(weekPlan,prevWeekPlan){const result=[];for(const plan of[weekPlan,prevWeekPlan].filter(Boolean)){for(const d of DAYS){const dp=plan?.[d.key]||{};for(const m of MEALS){const n=normalizeSlot(dp[m]);if(!n||n.leftoverFrom)continue;const dayLabel=`${d.label}${plan===prevWeekPlan?' (last wk)':''}`;if(n.recipeId)result.push({day:d.key,dayLabel,meal:m,recipeId:n.recipeId,multiplier:n.multiplier});else if(n.slotType==='eating_out')result.push({day:d.key,dayLabel,meal:m,isEatingOut:true,label:n.label||'Eating out'});}}}return result;}
  ...
  function renderDay(d){
    ...
    return(
      <div className="bg-white border border-stone-200 rounded-2xl p-3">
        ...
        <div className="space-y-2">
          {MEALS.map(m=>{
            const slot=normalizeSlot(dp[m]);const isLO=!!slot?.leftoverFrom;const origin=isLO?getOriginSlot(week,slot,prevWeek):slot;const isEatingOut=slot?.slotType==='eating_out'||(isLO&&origin?.slotType==='eating_out');const rid=origin?.recipeId,recipe=rid?recipes[rid]:null,orphaned=isLO&&!recipe&&!isEatingOut;const sk=`${d.key}_${m}`,ck=isLO&&slot?.leftoverFrom?`${slot.leftoverFrom.day}_${slot.leftoverFrom.meal}`:sk;const isCooked=!!cookedSlots[ck],mul=origin?.multiplier||1;const originEoLabel=isLO&&origin?.slotType==='eating_out'?origin.label||'Eating out':null;const olabel=isLO&&slot?.leftoverFrom?`${DAYS.find(x=>x.key===slot.leftoverFrom.day)?.label.slice(0,3)} ${slot.leftoverFrom.meal}`:null;
            return(
              <div key={m} className={`border rounded-lg p-2 min-h-[64px] ${isCooked?'border-emerald-200 bg-emerald-50/40':isEatingOut?'border-amber-200 bg-amber-50/30':isLO?'border-stone-100 bg-stone-50/40':'border-stone-100'}`}>
                ...
                {isEatingOut?(<div className="flex items-center justify-between gap-1"><div className="flex items-center gap-1.5"><UtensilsCrossed className="w-3.5 h-3.5 text-amber-600" strokeWidth={1.75}/><span className="font-display text-sm text-amber-800">{isLO?(originEoLabel||'Eating out'):(slot.label||'Eating out')}</span>{isLO&&olabel&&<span className="text-[10px] text-amber-600 ml-1">from {olabel}</span>}</div><button onClick={()=>onClearSlot(d.key,m)} className="text-stone-300 hover:text-red-500"><X className="w-3 h-3"/></button></div>)
...
function RecipePickerModal({recipes,recipeMap,title,currentWeekPlan,prevWeekPlan,targetSlot,onPick,onPickLeftover,onPickEatingOut,onClose}){
  ...
  return(
    <Modal onClose={onClose}>
      ...
      {mode==='leftover'&&<div className="max-h-96 overflow-y-auto space-y-1">{origins.length===0&&<p className="text-sm text-stone-500 py-6 text-center">No meals this week to use as leftovers.</p>}{origins.map(o=>{if(o.isEatingOut)return(<button key={`${o.day}_${o.meal}`} onClick={()=>onPickLeftover(o.day,o.meal)} className="w-full text-left p-3 rounded-lg hover:bg-stone-100 flex items-center gap-2"><UtensilsCrossed className="w-3.5 h-3.5 text-amber-500 flex-shrink-0"/><div><div className="text-[10px] uppercase tracking-wider text-stone-500">{o.dayLabel} · {MEAL_LABELS[o.meal]||o.meal}</div><div className="font-display text-base text-amber-800">{o.label}</div></div></button>);const r=recipeMap[o.recipeId];if(!r)return null;return(<button key={`${o.day}_${o.meal}`} onClick={()=>onPickLeftover(o.day,o.meal)} className="w-full text-left p-3 rounded-lg hover:bg-stone-100"><div className="text-[10px] uppercase tracking-wider text-stone-500">{o.dayLabel} · {MEAL_LABELS[o.meal]||o.meal}</div><div className="font-display text-base">{r.name}</div></button>);})}</div>}
