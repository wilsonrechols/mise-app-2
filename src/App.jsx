...
import {
  Sparkles, Send, X, ChevronDown, ChevronUp, Check,
  RefreshCw, Loader2, UtensilsCrossed, ChevronRight,
  MessageSquare, Zap
} from 'lucide-react';

// ─── pulled from App.jsx (already available globally in that file) ───────────
// DAYS, MEALS, generateId, getWeekStart, shiftWeek, normalizeSlot,
// getTodayDayKey, callAI, extractJSON
// We re-declare the ones we need so this file is self-contained for review;
// when merged into App.jsx, remove these and use the shared ones.
...
/** Turn a flat {mon:{breakfast,lunch,dinner},...} plan into a display-friendly array */
function planToRows(plan, recipes) {
  return DAYS_KEYS.map(dk => {
    const dp = (plan && plan[dk]) || {};
    return {
      dayKey: dk,
      dayLabel: DAYS_LABEL[dk],
      breakfast: resolveSlot(dp.breakfast, recipes),
      lunch:     resolveSlot(dp.lunch,     recipes),
      dinner:    resolveSlot(dp.dinner,    recipes),
    };
  });
}
...
function resolveSlot(slot, recipes) {
  ...
  if (typeof slot === 'string') {
    ...
    return { type: 'recipe', label: r ? r.name : slot, recipeId: slot };
  }
...
// Local copy so this file is self-contained (same logic as App.jsx)
...
/**
 * Renders the inline 7-day plan returned by the AI.
 * `draftPlan` shape: { mon: {breakfast, lunch, dinner}, ... }
 * where each value is either null | "recipeId" | {slotType:'eating_out',label} | {recipeId,…}
 */
                ...
                <div className="flex-1 grid grid-cols-3 gap-1.5">
                  {[breakfast, lunch, dinner].map((slot, mi) => {
                    ...
                    return (
                      <button
                        key={mealName}
                        onClick={() => onSwap(dayKey, mealName, slot)}
                        className={`text-left px-2 py-1.5 rounded-lg text-xs transition-colors group
                          ${slot.type === 'empty'
                            ? 'border border-dashed border-stone-200 text-stone-400 hover:border-stone-400 hover:text-stone-600'
                            : slot.type === 'out'
                              ? 'bg-amber-50 text-amber-800 border border-amber-100 hover:bg-amber-100'
                              : 'bg-stone-50 text-stone-700 border border-stone-100 hover:bg-stone-100'
                          }`}
                      >
                        <div className="text-[9px] uppercase tracking-wider mb-0.5 opacity-60">{mealName}</div>
                        <div className="leading-tight truncate">
                          {slot.type === 'empty' ? '+ add' : slot.label}
                        </div>
                      </button>
                    );
                  })}
...
function SlotSwapModal({ day, meal, current, recipeList, onSave, onClose }) {
  const [search, setSearch] = useState('');
  const [mode, setMode]     = useState('recipe'); // 'recipe' | 'out' | 'clear'
  ...
  return (
    ...
    >
      <div
        className="bg-stone-50 rounded-2xl p-4 w-full max-w-sm"
      ...
      >
        ...
        <div className="flex gap-1 mb-3 p-1 bg-stone-100 rounded-full">
          {[['recipe','Recipe'],['out','Eating out'],['clear','Clear']].map(([id,lbl]) => (
            <button
              key={id}
              onClick={() => setMode(id)}
              className={`flex-1 px-2 py-1 rounded-full text-xs font-medium ${mode === id ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500'}`}
            >
              {lbl}
            </button>
          ))}
        ...
        {mode === 'recipe' && (
          <>
            <input
              ...
              className="w-full px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm mb-2 focus:outline-none focus:border-stone-400"
            />
            <div className="max-h-56 overflow-y-auto space-y-1">
              {filtered.map(r => (
                <button
                  key={r.id}
                  onClick={() => onSave({ recipeId: r.id, slotType: 'recipe', multiplier: 1 })}
                  className="w-full text-left px-3 py-2 rounded-lg hover:bg-stone-100 text-sm"
                >
                  {r.name}
                  <span className="text-xs text-stone-400 ml-2">{r.cuisine}</span>
                </button>
              ))}
              {filtered.length === 0 && (
                <p className="text-sm text-stone-400 text-center py-4">No matches</p>
              )}
            </div>
          </>
        )}
        {mode === 'out' && (
          <div className="space-y-3">
            <input
              autoFocus
              value={eoLabel}
              onChange={e => setEoLabel(e.target.value)}
              placeholder="e.g. Sushi night, Tacos…"
              className="w-full px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400"
              onKeyDown={e => e.key === 'Enter' && onSave({ slotType: 'eating_out', label: eoLabel || 'Eating out' })}
            />
            <button
              onClick={() => onSave({ slotType: 'eating_out', label: eoLabel || 'Eating out' })}
              className="w-full py-2 rounded-full bg-amber-600 text-white text-sm hover:bg-amber-700"
            >
              Set as eating out
            </button>
          </div>
        ...
        {mode === 'clear' && (
          <button
            onClick={() => onSave(null)}
            className="w-full py-2 rounded-full bg-stone-200 text-stone-700 text-sm hover:bg-stone-300"
          >
            Clear this slot
          </button>
        )}
...
function ChatMessage({
  msg, recipes, recipeList, onApplyPlan, onStartCooking,
  applying, swapTarget, onSwapOpen, onSwapClose, onSwapSave,
}) {
  ...
  if (isUser) {
    return (
      <div className="flex justify-end mb-3">
        <div className="bg-stone-900 text-stone-50 rounded-2xl rounded-br-sm px-4 py-2.5 text-sm max-w-[80%]">
          {msg.content}
        </div>
      </div>
    );
    ...
    <div className="flex gap-2.5 mb-4 items-start">
      ...
      <div className="flex-1 min-w-0">
        {msg.loading ? (
          ...
          <>
            {msg.text && (
              <div className="text-sm text-stone-800 leading-relaxed whitespace-pre-wrap">{msg.text}</div>
            )}
            ...
            {/* "Save as recipe?" prompt after free-text log */}
            {msg.saveAsRecipePrompt && (
              <div className="mt-3 flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2.5">
                <span className="text-xs text-amber-800 flex-1">
                  Want to save "<strong>{msg.saveAsRecipePrompt}</strong>" as a recipe?
                </span>
                <button
                  onClick={() => msg.onSaveAsRecipe && msg.onSaveAsRecipe()}
                  className="flex-shrink-0 px-3 py-1 rounded-full bg-amber-600 text-white text-xs hover:bg-amber-700"
                >
                  Save
                </button>
              </div>
            ...
            {msg.recipeLinks && msg.recipeLinks.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {msg.recipeLinks.map(({ id, name }) => (
                  <button
                    key={id}
                    onClick={() => onStartCooking(id)}
                    className="flex items-center gap-1 px-2.5 py-1 rounded-full border border-orange-200 text-orange-700 text-xs hover:bg-orange-50"
                  >
      ...
      {/* Swap modal, rendered at message level so it's scoped */}
      {swapTarget && swapTarget.msgId === msg.id && (
        <SlotSwapModal
          day={swapTarget.day}
          meal={swapTarget.meal}
          current={swapTarget.slot}
...
export function MealChat({
  ...
  const [applying, setApplying] = useState(null);   // msgId being applied
  const [swapTarget, setSwapTarget] = useState(null); // { msgId, day, meal, slot }
  const [draftPlans, setDraftPlans] = useState({});   // msgId → mutable plan object
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      if (intent === 'plan') {
        const prompt = `
You are a helpful meal planning assistant for the app "Mise".
...
`.trim();
        ...
        // Store a mutable copy of this draft so swap works
        setDraftPlans(prev => ({ ...prev, [msgId]: JSON.parse(JSON.stringify(msgDraftPlan)) }));
      ...
      // ── PREP intent ────────────────────────────────────────────────────
        ...
        const prompt = `
You are a meal prep coach inside the app "Mise".
...
`.trim();
        ...
        let recipeLinks = [];
        let bodyText    = raw.replace(/RECIPE_LINKS:.*$/s, '').trim();

        if (linkMatch) {
          try {
            const ids = JSON.parse(linkMatch[1]);
            recipeLinks = ids
              .map(id => ({ id, name: recipes[id]?.name }))
              .filter(r => r.name);
          } catch {/* ignore */}
        }
      ...
      // ── LOG intent ─────────────────────────────────────────────────────
        ...
        const prompt = `
You are a meal logging assistant inside "Mise".
...
`.trim();
        ...
        onAddMealHistory({
          ...
          slotType: data.recipeId ? 'recipe' : 'freetext',
        });

        // Check if we should offer "save as recipe"
        const isNew = data.isNewFood && !data.recipeId;
        const saveLabel = data.label || trimmed;

        updateMessage(replyId, {
          loading:            false,
          text:               data.reply || `Logged "${saveLabel}"!`,
          saveAsRecipePrompt: isNew ? saveLabel : null,
          onSaveAsRecipe:     isNew ? () => onSaveRecipe({
            name:         saveLabel,
            servings:     2,
            prepTime:     15,
            cookTime:     30,
      ...
      // ── GENERAL ────────────────────────────────────────────────────────
        ...
        const prompt = `
You are a helpful cooking and meal planning assistant inside the app "Mise".
...
`.trim();
  ...
  async function handleApplyPlan(msgId, plan) {
    ...
    setApplying(msgId);
    try {
      onApplyWeekPlan(currentWeek, livePlan);
      updateMessage(msgId, { text: '✓ Plan applied to your week!', draftPlan: null });
    } finally {
      setApplying(null);
    }
  }
  ...
  function handleSwapSave(msgId, day, meal, newSlot) {
    // Mutate the draft plan for this message
    setDraftPlans(prev => {
      const plan = { ...(prev[msgId] || {}) };
      plan[day] = { ...(plan[day] || {}) };
      plan[day][meal] = newSlot;
      return { ...prev, [msgId]: plan };
    });
    // Also update the message's draftPlan reference so preview re-renders
    setMessages(prev => prev.map(m => {
      if (m.id !== msgId || !m.draftPlan) return m;
      const updated = { ...m.draftPlan, [day]: { ...(m.draftPlan[day] || {}), [meal]: newSlot } };
      return { ...m, draftPlan: updated };
    }));
    setSwapTarget(null);
  }
  ...
  return (
    <>
      {/* ── Closed state: chat trigger card ─────────────────────────── */}
      {!open && (
        <button
          onClick={() => setOpen(true)}
          className="w-full text-left bg-white border border-stone-200 rounded-2xl p-5 mb-4 hover:border-orange-300 hover:bg-orange-50/30 transition-colors group"
        >
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-orange-600" />
              <h3 className="font-display text-lg">Meal planning assistant</h3>
            </div>
            <ChevronDown className="w-4 h-4 text-stone-400 group-hover:text-orange-600 transition-colors" />
          </div>
          <p className="text-sm text-stone-500">
            "Plan my week", "What can I prep tonight?", quick-log a meal…
          </p>
        </button>
      )}
      ...
      {open && (
        <div className="bg-white border border-stone-200 rounded-2xl mb-4 flex flex-col overflow-hidden" style={{ maxHeight: '70vh' }}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-stone-100 flex-shrink-0">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-orange-600" />
              <span className="font-display text-base">Meal assistant</span>
            </div>
            <button
...
// ============================================================
//
// 1. Add this file as  src/MealChat.jsx
//
// 2. In App.jsx, import at the top:
//    import { MealChat } from './MealChat';
//
// 3. Add the applyWeekPlanFromAI handler in the App() function
//    (alongside your other state functions):
//
//    function applyWeekPlanFromAI(weekKey, aiPlan) {
//      setState(s => {
//        const existing = s.mealPlan[weekKey] || {};
//        const merged   = { ...existing };
//        for (const dk of DAYS.map(d => d.key)) {
//          if (!aiPlan[dk]) continue;
//          merged[dk] = { ...(merged[dk] || {}) };
//          for (const meal of MEALS) {
//            const slot = aiPlan[dk][meal];
//            if (slot === undefined) continue;  // don't overwrite if AI left it out
//            if (slot === null) {
//              delete merged[dk][meal];
//            } else if (typeof slot === 'string') {
//              merged[dk][meal] = { recipeId: slot, multiplier: 1, slotType: 'recipe' };
//            } else {
//              merged[dk][meal] = { multiplier: 1, slotType: 'recipe', ...slot };
//            }
//          }
//        }
//        return { ...s, mealPlan: { ...s.mealPlan, [weekKey]: merged } };
//      });
//    }
//
// 4. In HomeView's props, pass the new handler + meal data needed:
//
//    {view === 'home' && <HomeView
//      ...existing props...
//      mealHistory={state.mealHistory}
